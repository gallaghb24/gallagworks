...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const { data: assessment, error } = await supabase
      .from("assessments")
      .select("id, total_score, maturity_level, leads(name, email, organisation)")
      .eq("id", assessment_id)
      .maybeSingle();
    ...
    const resultsUrl = `https://www.gallag.works/diagnostic/results/${encodeURIComponent(assessment.id)}`;
...
<body style="margin:0;padding:0;background-color:#121212;color:#F5F5F5;font-family:Arial,Helvetica,sans-serif;line-height:1.6;">
  <div style="max-width:600px;margin:0 auto;padding:40px 24px;">
    ...
    return new Response(JSON.stringify({ error: "Failed to send consultation request" }), {

...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const { data: assessment, error } = await supabase
      .from("assessments")
      .select("id, total_score, maturity_level, dimension_scores, leads(name, email, organisation, role, industry, company_size)")
      .eq("id", assessment_id)
      .maybeSingle();
    ...
    const lead = (assessment as any).leads;
    ...
    const industry = lead.industry ? String(lead.industry) : null;
    const company_size = lead.company_size ? String(lead.company_size) : null;
    const total_score = Number(assessment.total_score ?? 0);
    const maturity_level = String(assessment.maturity_level ?? "");
    const dimension_scores = (assessment.dimension_scores ?? {}) as Record<string, number>;

    const safeName = escapeHtml(name.trim());
    const safeOrg = escapeHtml(organisation.trim());
    const { strongest, weakest } = getStrongestAndWeakest(dimension_scores);
    const safeAssessmentId = encodeURIComponent(assessment.id);
    const resultsUrl = `https://www.gallag.works/diagnostic/results/${safeAssessmentId}`;
...
<body style="margin:0;padding:0;background-color:#121212;color:#F5F5F5;font-family:Arial,Helvetica,sans-serif;line-height:1.6;">
  <div style="max-width:600px;margin:0 auto;padding:40px 24px;">
    ...
    <table style="width:100%;border-collapse:collapse;margin:0 0 24px 0;">
      <tr><td style="padding:8px 12px;font-size:13px;color:#888;font-family:'Courier New',Courier,monospace;border-bottom:1px solid #2F3133;">Strongest dimension</td><td style="padding:8px 12px;font-size:13px;color:#F5F5F5;font-family:'Courier New',Courier,monospace;border-bottom:1px solid #2F3133;text-align:right;">${escapeHtml(DIMENSION_LABELS[strongest.key] || strongest.key)} (${strongest.score})</td></tr>
      <tr><td style="padding:8px 12px;font-size:13px;color:#888;font-family:'Courier New',Courier,monospace;">Needs most attention</td><td style="padding:8px 12px;font-size:13px;color:#F5F5F5;font-family:'Courier New',Courier,monospace;text-align:right;">${escapeHtml(DIMENSION_LABELS[weakest.key] || weakest.key)} (${weakest.score})</td></tr>
    </table>
    <p style="margin:0 0 24px 0;"><a href="${resultsUrl}" style="display:inline-block;padding:12px 24px;background-color:#FF5F1F;color:#FFFFFF;text-decoration:none;font-size:14px;font-weight:700;letter-spacing:0.5px;">VIEW FULL RESULTS</a></p>
    ...
    const dimensionRows = Object.entries(dimension_scores)
      .map(([key, score]) => `<tr><td style="padding:4px 8px;font-size:13px;color:#F5F5F5;">${escapeHtml(DIMENSION_LABELS[key] || key)}</td><td style="padding:4px 8px;font-size:13px;color:#FF5F1F;text-align:right;">${score}/25</td></tr>`)
      .join("");
...
<body style="margin:0;padding:0;background-color:#121212;color:#F5F5F5;font-family:Arial,Helvetica,sans-serif;line-height:1.6;">
  <div style="max-width:600px;margin:0 auto;padding:32px 24px;">
    ...
    <table style="width:100%;border-collapse:collapse;margin:8px 0 24px 0;">${dimensionRows}</table>
    <p style="margin:0;">
